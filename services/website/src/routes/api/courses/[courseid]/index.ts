...
import { readFileYaml, readLessonConfigFiles } from "$lib/github-graphql-queries";
...
export const get: RequestHandler = async ({params}) => {
  ...
  const courseConfig = await readFileYaml<CourseConfig>(octokit, `framework/${params.courseid}/config.yaml`)
  const courseLessonConfigFiles = await readLessonConfigFiles(octokit, `framework/${params.courseid}/lessons`)
